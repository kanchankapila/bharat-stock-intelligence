...
## Screeners are DESCRIPTIVE, not predictive - and the intraday version is untestable today
...
- **The genuinely open version - enter at the moment of flagging, exit at that day's close - CANNOT be tested with current data.** `screener_appearances.appeared_date` is `TIMESTAMPTZ` but every row is written at `00:00:00`: exactly one distinct time across 720,824 rows. The capture time is never recorded. `intraday_ohlcv` is ready for it (15-minute bars, 2,353 symbols, **215 days**), so the price side is not the blocker.
