...
-- ── et_cashflow_history ──────────────────────────────────────────────
-- Full 6-year annual CFO/CFI/CFF series per stock, harvested 2026-09-01 from the ET_Stats
-- CashFlow payload financial_ratios_fetcher.py was ALREADY fetching at last=6 (only period
-- [0] was persisted before; the other 5 annual periods were discarded). Written by the same
-- fetcher (weekly cadence); PK (symbol, year_ending) makes weekly runs converge in place.
...
-- QUARTERLY cash-flow history (2026-09-01): fetched weekly by finstack_cashflow_fetcher.py,
-- which speaks MCP stdio to `python -m finstack.server` and calls its cash_flow tool
-- (yfinance quarterly_cashflow underneath). Vendor coverage is a subset of NSE names;
-- absent symbols mean no vendor data, not stale data. PK (symbol, period_end) makes the
-- weekly cadence converge in place.
