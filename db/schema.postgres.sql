...
CREATE TABLE IF NOT EXISTS "bulk_block_deals" (
  ...
  "source" TEXT NOT NULL DEFAULT 'nse'::text,
...
-- ── dalalos_financial_trends_history ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS "dalalos_financial_trends_history" (
  ...
  "source" TEXT NOT NULL DEFAULT 'dalalos-mcp'::text,
  "fetched_at" TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY ("symbol", "period_end", "period_type")
);
CREATE INDEX idx_dalalos_financial_trends_symbol ON public.dalalos_financial_trends_history USING btree (symbol);
...
CREATE TABLE IF NOT EXISTS "et_cashflow_history" (
  ...
  "cfi" DOUBLE PRECISION,
  "cff" DOUBLE PRECISION,
  "fetched_at" TEXT DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY ("symbol", "year_ending")
);
CREATE INDEX idx_ecf_sym ON public.et_cashflow_history USING btree (symbol, year_ending DESC);
...
CREATE TABLE IF NOT EXISTS "feature_store" (
  ...
  "analyst_target_upside_pct" DOUBLE PRECISION,
  "analyst_n" DOUBLE PRECISION,
  "broker_recos_90d" DOUBLE PRECISION,
  "days_to_next_earnings" DOUBLE PRECISION,
  "days_since_last_earnings" DOUBLE PRECISION,
  "last_eps_surprise_pct" DOUBLE PRECISION,
  "last_beat_score" DOUBLE PRECISION,
  "earnings_in_5d" DOUBLE PRECISION,
  "delivery_z_20d" DOUBLE PRECISION,
...
CREATE INDEX idx_fch_sym ON public.finstack_cashflow_history USING btree (symbol, period_end DESC);
...
CREATE INDEX idx_job_sweep_results_sweep ON public.job_sweep_results USING btree (sweep_id, started_at DESC);
...
CREATE TABLE IF NOT EXISTS "news_sentiment_items" (
  ...
  "tone_neu" DOUBLE PRECISION,
  "tone_label" TEXT,
  "tone_score" DOUBLE PRECISION,
  "sentiment_conflict" DOUBLE PRECISION
);
...
-- ── niftytrader_live_screener_snapshots ─────────────────────────────────────────────
...
CREATE INDEX idx_nt_live_screener_filter_date ON public.niftytrader_live_screener_snapshots USING btree (filter_name, trade_date, captured_at);
...
-- ── ohlcv_corporate_actions_checked ─────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS "technical_signals" (
  ...
  "preopen_volume" DOUBLE PRECISION,
  PRIMARY KEY ("symbol", "date")
);
