...
- **Still open (top)**: P0 ensemble.pkl has no promotion bar (auto-activates every retrain); P0 `technicalSignalsService.ts:1058` OHLCV scan query still unbounded by `scanDate` (future-bar leak on historical scans); label redesign (WIN/LOSS→3-class); promote breakout_probability into the ranker. 3 fetchers vendor-blocked (AMFI mf-sector-flow, ET mf-holdings, Trendlyne cash-flow params) — awaiting alternative URLs from user.
