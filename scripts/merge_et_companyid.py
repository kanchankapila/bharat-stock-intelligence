#!/usr/bin/env python3
"""Merge scripts/et_mappings.json (from fetch_et_companyid_mappings.py) into
stocklist.json / stocklist.ts / stocklist.js, filling in only missing
`companyid` values -- mirrors merge_finology.py.
"""
import os
import json
...
ET_MAPPINGS_PATH = os.path.join(SCRIPTS_DIR, "et_mappings.json")


def main():
    if not os.path.exists(ET_MAPPINGS_PATH):
        print(f"Error: {ET_MAPPINGS_PATH} does not exist. Run fetch_et_companyid_mappings.py first.")
        return
    ...
    with open(ET_MAPPINGS_PATH, "r", encoding="utf-8") as f:
        et_list = json.load(f)

    et_map = {}
    for entry in et_list:
        sym = entry.get("symbol")
        if sym and entry.get("companyid"):
            et_map[sym.upper()] = entry.get("companyid")
    ...
    for entry in stock_list:
        sym = entry.get("symbol")
        if sym and not entry.get("companyid"):
            sym_upper = sym.strip().upper()
            if sym_upper in et_map:
                entry["companyid"] = et_map[sym_upper]
                updated_count += 1

    print(f"Successfully filled companyid for {updated_count} stocks in stocklist.json")
