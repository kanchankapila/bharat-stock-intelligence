...
def _load_stocks(symbol_filter: str | None, con) -> list[tuple[str, str]]:
    """Return [(symbol, tlid), ...] from trendlyne_screener_stocks.

    Uses DISTINCT to avoid duplicate API calls when the same stock appears in
    multiple screeners. Filters to rows where stock_id is populated (tlid).
    """
    cur = con.cursor()
    cur.execute("""
