...
def test_first_run_writes_the_whole_series():
    conn = _db()
    assert write_technical_history(conn, "TESTSYM", SERIES, "2026-08-12", known={}) == 3


def test_skips_dates_already_stored():
    conn = _db()
    write_technical_history(conn, "TESTSYM", SERIES, "2026-08-11",
                            known={("TESTSYM", "indigraph", "d"): "2026-08-11"})
    known = load_known_max_dates(conn)
    ...
    assert write_technical_history(conn, "TESTSYM", SERIES, "2026-08-13", known=known) == 0


def test_full_rerun_still_upserts_everything():
    """--full passes known=None -- a vendor restatement has to be able to overwrite history."""
    conn = _db()
    write_technical_history(conn, "TESTSYM", SERIES, "2026-08-12", known={})
    assert write_technical_history(conn, "TESTSYM", SERIES, "2026-08-13", known=None) == 3
    row = conn.execute(
...
def test_unseen_series_is_not_skipped_by_another_symbols_high_water_mark():
    """The guard keys on (symbol, indicator, period); a busy symbol must not suppress a new one."""
    conn = _db()
    known = {("OTHERSYM", "indigraph", "d"): "2026-08-12"}
    assert write_technical_history(conn, "TESTSYM", SERIES, "2026-08-12", known=known) == 3
