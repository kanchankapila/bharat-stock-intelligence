...
export function AgentAuditorPage() {
  ...
  return (
    <div className="v1-page space-y-6">
      <div className="v1-header">
        <div className="v1-header-left">
          <h1 className="v1-title-page flex items-center gap-2.5">
            <BarChart2 className="w-6 h-6 text-amber-400" /> Auditor Agent
          </h1>
          {report && <p className="text-sm text-slate-400 mt-1 font-data">Auditing picks from: {report.audit_for_date}</p>}
        </div>
      ...
      {isLoading && <div className="v1-card p-6 text-sm text-slate-400 font-data animate-pulse">Loading audit...</div>}

      {report && (
        <>
          <div className="v1-card p-5">
            <p className="v1-title-card text-amber-300 mb-2">📋 Audit Report — {TF_LABELS[tf]}</p>
            <p className="text-slate-100 text-sm leading-relaxed">{report.narrative || 'No narrative.'}</p>
          </div>
          ...
          <div className="v1-grid-4">
            {[
              ...
              { label: 'Profit Factor', value: report.profit_factor?.toFixed(2), color: report.profit_factor >= 1.5 ? 'text-emerald-400' : 'text-amber-400' },
            ].map(m => (
              <div key={m.label} className="v1-card p-4">
                <p className="v1-data-label">{m.label}</p>
                <p className={`v1-data-value text-2xl mt-1 ${m.color}`}>{m.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
              ...
              { label: '⏳ Open', value: report.open_positions, color: 'text-amber-400' },
            ].map(m => (
              <div key={m.label} className="v1-card p-4">
                <p className="v1-data-label">{m.label}</p>
                <p className={`v1-data-value text-xl mt-1 ${m.color}`}>{m.value}</p>
              </div>
            ))}
          </div>
                ...
                <BarChart data={attrData} layout="vertical">
                  ...
                  <Bar dataKey="wr" name="Win Rate %">
                    {attrData.map((entry, i) => (
                      <Cell key={i} fill={entry.wr >= 60 ? '#10b981' : entry.wr >= 45 ? '#f59e0b' : '#f43f5e'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
      ...
      {!report && !isLoading && (
        <div className="v1-card p-6 text-slate-400 text-sm font-data">No audit data for {TF_LABELS[tf]} yet. Run the agent after market close.</div>
      )}
    </div>
  );
}
