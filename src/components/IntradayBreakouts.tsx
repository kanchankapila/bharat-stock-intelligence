...
export const IntradayBreakouts: React.FC<IntradayBreakoutsProps> = ({ onSelectStock }) => {
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 col-span-12">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Live Breakout Detection</p>
        </div>
      </div>

      {breakouts.length === 0 ? (
        <div className="py-8 text-center">
          <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest italic">No breakout data currently detected</p>
        </div>
      ) : (
          ...
          {breakouts.slice(0, 12).map((stock: any) => {
            ...
            return (
              <div 
                ...
                className="bg-slate-950 border border-slate-800 rounded-2xl p-4 cursor-pointer hover:border-slate-700 transition-all group"
              >
                <div className="flex justify-between items-start mb-3">
                  <p className="text-sm font-black text-white italic uppercase tracking-wider group-hover:text-blue-400 transition-colors">
                    {stock.symbol_name}
                  </p>
                  <div className={cn(
                ...
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">LTP</p>
                    <p className="text-lg font-black text-white tabular-nums tracking-tighter">₹{stock.last_trade_price}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Vol Ratio</p>
