...
import { extractIndexId } from '../lib/utils';
  ...
  return (
    <Card title="Key Market Indices" icon={Activity} className={className}>
      <div ref={ref}>
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 pt-2">
            {[0, 1, 2, 3, 4, 5].map(i => <div key={i} className="h-20 bg-slate-900/50 animate-pulse rounded-2xl" />)}
          </div>
        ) : settledEmpty ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {keyList.map((idx: any) => {
          const indexId = extractIndexId(idx.url);
