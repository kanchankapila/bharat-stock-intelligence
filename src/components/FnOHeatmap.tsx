...
const FnOHeatmap: React.FC<{ onSelectStock: (s: string) => void }> = ({ onSelectStock }) => {
  ...
  if (isLoading) return <div className="h-96 bg-[#141416]/80 animate-pulse rounded-3xl" />;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[8px] font-bold text-zinc-600 uppercase">Longs</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 bg-rose-500 rounded-sm" />
            <span className="text-[8px] font-bold text-zinc-600 uppercase">Shorts</span>
          </div>
        </div>
      </div>
      ...
      {stocks.length > 0 ? (
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-10 lg:grid-cols-12 gap-1">
          {stocks.map((s: any, idx: number) => (
            ...
            >
              ...
              {/* Tooltip on hover */}
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 p-2 bg-[#141416] border border-white/[0.07] rounded-lg opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-50 shadow-2xl">
                <p className="text-[10px] font-black text-white truncate">{s.name}</p>
                <div className="flex justify-between mt-1">
                  <span className="text-[8px] font-bold text-zinc-500">Price:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-[8px] font-bold text-zinc-500">OI Chg:</span>
        ...
        <div className="h-48 flex flex-col items-center justify-center bg-[#141416]/50 border border-white/[0.05] rounded-3xl">
           <Activity className="w-8 h-8 text-zinc-800 mb-3" />
           <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Sectoral heatmap currently unavailable</p>
        </div>
      )}
    </div>
  );
