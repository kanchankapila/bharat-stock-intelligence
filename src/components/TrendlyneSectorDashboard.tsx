...
export const TrendlyneSectorDashboard: React.FC = () => {
  ...
  const renderSortIcon = (field: string) => {
    ...
    return sortDir === 'desc' ? <TrendingDown className="w-3 h-3 ml-1 text-blue-400" /> : <TrendingUp className="w-3 h-3 ml-1 text-blue-400" />;
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800">
                {sortedItems.map((s) => (
                  <tr key={s.name.stockName} className="hover:bg-slate-800/30 transition-colors">
                    <td className="p-4">
