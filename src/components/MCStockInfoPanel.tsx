...
export const MCStockInfoPanel: React.FC<MCStockInfoPanelProps> = ({ 
  ...
  if (!isVisible && !unifiedData) {
    return (
      <div ref={containerRef} className="h-40 flex items-center justify-center bg-[#141416]/10 border border-dashed border-white/[0.07] rounded-2xl">
    ...
    return (
      <div ref={containerRef} className="space-y-4 animate-pulse">
        <div className="flex gap-2 mb-4">
          {[1,2,3].map(i => <div key={i} className="h-8 w-20 bg-white/[0.08] rounded-lg" />)}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1,2,3,4,5,6,7,8].map(i => <div key={i} className="h-20 bg-white/[0.08]/50 rounded-2xl" />)}
        </div>
  ...
  if (error || !unifiedData) {
    ...
    return (
      <div ref={containerRef} className="p-8 text-center bg-[#141416]/50 border border-white/[0.07] rounded-2xl">
        <AlertCircle className="w-8 h-8 text-rose-500 mx-auto mb-3" />
        <p className="text-sm text-zinc-500 font-bold">Failed to load data for {stockName}</p>
        <p className="text-[10px] text-zinc-600 mt-1 uppercase tracking-widest">{symbol}</p>
      </div>
    );
  }
  ...
  if (!hasAnyData) {
    ...
    return (
      <div ref={containerRef} className="p-8 text-center bg-[#141416]/50 border border-white/[0.07] rounded-2xl">
        ...
        <p className="text-sm text-zinc-400 font-bold">No MoneyControl data available for {stockName}</p>
        <p className="text-[10px] text-zinc-600 mt-1 uppercase tracking-widest">{symbol} — not mapped to a MoneyControl ID</p>
      </div>
    );
  }
    ...
    <div ref={containerRef} className="space-y-4">
      ...
      {(!section || section === 'all' || section === 'overview') && (
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 pr-4 border-r border-white/[0.07]">
              <Zap className="w-4 h-4 text-violet-500" />
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">AlphaQuant V2</span>
              {alphaData && (
                <div className="flex items-center gap-2">
                  <span className={cn(
                    "text-[9px] font-black px-2 py-0.5 rounded uppercase",
                    alphaData.score.score >= 70 ? "bg-blue-500/20 text-violet-400" :
                    alphaData.score.score >= 50 ? "bg-white/[0.08] text-zinc-400" :
                    "bg-rose-500/10 text-rose-500"
                  )}>
                    Rank: #{alphaData.score.score.toFixed(1)}
                  </span>
                  <span className="text-[8px] font-black text-violet-500/60 uppercase">{alphaData.score.classification}</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-2">
              <BrainCircuit className="w-4 h-4 text-emerald-500" />
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">MoneyControl</span>
              {classification && (
                <span className={cn(
                  "text-[9px] font-black px-2 py-0.5 rounded uppercase",
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center gap-2 pl-4 border-l border-white/[0.07]">
                  <BarChart3 className="w-4 h-4 text-violet-500" />
                  <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">TradeBrains</span>
                  <span className={cn(
                    "text-[9px] font-black px-2 py-0.5 rounded uppercase",
          ...
          <div className="flex gap-1 bg-[#141416] rounded-lg p-0.5 border border-white/[0.07]">
            {(['D', 'W', 'M'] as Timeframe[]).map(tf => (
              <button
                ...
                className={cn(
                  ...
                  timeframe === tf ? "bg-blue-600 text-white shadow-lg" : "text-zinc-500 hover:text-white"
                )}
              >
      ...
      {(!section || section === 'all' || activeTab === 'overview') && (
        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
          ...
          <CompactMetricCard label="Market Cap" value={essentials?.marketCap || eq?.MKTCAP ? `₹${String(eq?.MKTCAP || essentials?.marketCap || '0').replace(/[^\d.]/g, '')}Cr` : '—'} color="text-violet-400" icon={Users} />
      ...
      {(!section || section === 'all') && (tb?.profile || tb?.overviewData?.stock_mentions) && (
        <div className="flex flex-wrap gap-3">
          {tb?.profile && (
            <div className="flex-1 min-w-0 flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2.5 bg-[#0c0c0e] border border-white/[0.07] rounded-xl">
              {tb.profile.founded_year && (
                <span className="text-[9px] font-bold text-zinc-500">Founded <span className="text-zinc-300 font-black">{tb.profile.founded_year}</span></span>
              )}
              {tb.profile.chairman && (
                <span className="text-[9px] font-bold text-zinc-500">Chairman <span className="text-zinc-300 font-black">{tb.profile.chairman}</span></span>
              )}
              {tb.profile.website && (
              ...
              {tb.profile.address && (
                <span className="text-[9px] font-bold text-zinc-600 truncate">{tb.profile.address}</span>
              )}
            </div>
          )}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            {alphaData && (
              <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3 flex flex-col justify-between min-h-[140px]">
                <p className="text-[9px] font-black text-violet-500 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                  <Zap className="w-3 h-3" /> AlphaQuant Core
                </p>
                <div className="grid grid-cols-5 gap-1">
                  {[
                    { label: 'TECH', value: alphaData.factors.technical,    color: 'text-violet-400'    },
                    ...
                    <div key={factor.label} className="text-center">
                      ...
                      <p className="text-[6px] font-black text-zinc-600 uppercase tracking-tighter">{factor.label}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-3">
                  <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-zinc-600 mb-1">
                    <span>Aggregate</span>
                    <span className="text-violet-400">{(alphaData.factors.momentum * 10).toFixed(0)}%</span>
                  </div>
                  <div className="h-0.5 w-full bg-[#141416] rounded-full overflow-hidden">
            ...
            {tb?.portalScore && (() => {
              ...
              const dims = [
                { label: 'Perf',   val: ps.performance,   color: 'bg-blue-500',    text: 'text-violet-400'    },
              ...
              return (
                <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3 min-h-[140px]">
                    ...
                    {dims.map((d) => (
                      <div key={d.label} className="space-y-0.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[7px] font-black text-zinc-500 uppercase tracking-tight">{d.label}</span>
            ...
            {(techD || techW || techM) && (
              <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3 min-h-[140px]">
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                  ...
                  {[
                    ...
                    return (
                      <div key={label} className={cn("p-1.5 rounded-lg border text-center",
                        ...
                        isBear ? "bg-rose-500/5 border-rose-500/10" : "bg-[#141416] border-white/[0.07]"
                      )}>
                        <p className="text-[7px] font-black text-zinc-600 uppercase mb-0.5">{label}</p>
                ...
                {techD && techW && techM && (() => {
                  ...
                  return (
                    <div className={cn("px-2 py-1 rounded-lg text-[8px] font-bold italic border flex gap-1.5 items-center",
                      mBull && dBear ? "bg-blue-500/5 text-violet-400 border-violet-500/10" : "bg-amber-500/5 text-amber-400 border-amber-500/10"
                    )}>
            ...
            {/* Active Signals List */}
            <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3 flex flex-col min-h-[140px]">
              <div className="flex items-center gap-1.5 mb-2">
                <Zap className="w-3 h-3 text-amber-400" />
                <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Active Signals ({allScreeners.length})</span>
              </div>
              <div className="grid grid-cols-2 gap-1.5 overflow-y-auto max-h-[85px] pr-1 scrollbar-none">
                {allScreeners.slice(0, 10).map((screener, i) => {
                  ...
                  return (
                    <div key={`${screener.id}-${i}`}
                      ...
                      className={cn("px-2 py-1 rounded-md text-[7px] font-black uppercase tracking-tighter border flex items-center gap-1.5 transition-all cursor-pointer hover:bg-[#141416] active:scale-95",
                        ...
                        "bg-[#141416] border-white/[0.07] text-zinc-400"
                      )}>
                ...
                {allScreeners.length === 0 && <p className="text-[8px] text-zinc-700 italic font-bold text-center mt-4">No active signals</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {classification?.longDesc && (
              <div className={cn("p-3 rounded-2xl border relative overflow-hidden flex flex-col justify-between",
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-[11px] text-zinc-300 font-medium italic leading-relaxed">{classification.longDesc}</p>
                </div>
                <div className="flex items-center gap-3 mt-3 relative z-10">
                  ...
                  )}>{classification.stockScore}<span className="text-[7px] text-zinc-600 font-bold ml-0.5">/100</span></span>
                </div>
              </div>
            )}
            ...
            {essentials?.checklist && (
              <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                    <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">MC Checklist</span>
                  </div>
                  {essentials.passText && (
                    <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                  {(['financials', 'industry', 'ownership', 'others'] as const).map((key) => {
                    ...
                    const colorMap = { financials: 'text-violet-500', industry: 'text-purple-500', ownership: 'text-amber-500', others: 'text-zinc-500' };
                    const items = essentials.checklist![key];
                    if (!items?.length) return null;
                    return (
                      <div key={key}>
                        <p className={cn("text-[7px] font-black uppercase tracking-widest mb-1.5 border-b border-white/[0.05] pb-0.5", colorMap[key])}>{labelMap[key]}</p>
                        <div className="grid grid-cols-1 gap-1">
                          {items.slice(0, 3).map((item, i) => (
                            <div key={i} className="flex items-center justify-between px-1.5 py-1 bg-[#141416]/50 rounded border border-white/[0.07]/20">
                              <span className="text-[8px] text-zinc-500 font-bold leading-tight truncate mr-2">{item.question}</span>
        ...
        <div className="space-y-6">
          ...
          {(fov || tb?.keyMetrics) && (
            <Card title="Valuation & Profitability Matrix" icon={BarChart3}>
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 pt-2">
                {[
                  { label: 'TTM EPS',    val: fov?.ttmEpsText || tb?.keyMetrics?.eps, color: 'text-violet-400' },
                  ...
                  { label: 'Ind. PE',    val: tb?.keyMetrics?.industry_pe,            color: 'text-zinc-400' },
                  ...
                  { label: 'EV/Sales',   val: tb?.keyMetrics?.ev_sales,               color: 'text-zinc-400' },
              ...
              <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3">
                ...
                <div className="space-y-1.5">
                  {detailedInsights.price.slice(0, 6).map((p: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-2 bg-[#141416]/50 rounded-lg border border-white/[0.04]">
                      <span className="text-[9px] font-black text-zinc-500 uppercase truncate max-w-[40%]">{p.shortDesc.split(' ')[0]}</span>
                      <span className={cn("text-[9px] font-black text-right flex-1", 
                        p.color === 'positive' ? "text-emerald-400" : p.color === 'negative' ? "text-rose-400" : "text-zinc-300"
                      )}>{p.shortDesc}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3">
                ...
                <div className="space-y-1.5">
                  {detailedInsights.shareholding.slice(0, 6).map((sh: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-2 bg-[#141416]/50 rounded-lg border border-white/[0.04]">
                      <span className="text-[9px] font-black text-zinc-500 uppercase truncate max-w-[40%]">{sh.shorttext}</span>
                      <span className={cn("text-[9px] font-black text-right flex-1",
                        sh.color === 'positive' ? "text-emerald-400" : sh.color === 'negative' ? "text-rose-400" : "text-zinc-300"
                      )}>{sh.longtext}</span>
                    </div>
                  ))}
                </div>
          ...
          <Card title="Performance & Returns Hub" icon={BarChart3}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
              <div className="space-y-4">
                <p className="text-[9px] font-black text-violet-500 uppercase tracking-widest border-l-2 border-violet-500 pl-2">MC Price Returns</p>
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(pv?.price || {}).map(([period, val]) => (
                    <div key={period} className="p-2.5 bg-[#0c0c0e] rounded-xl border border-white/[0.05] text-center">
                      <p className="text-[7px] font-black text-zinc-500 uppercase tracking-widest mb-0.5">{period}</p>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                      {[
                        ...
                        return (
                          <div key={label} className="p-2 bg-[#0c0c0e] rounded-lg border border-white/[0.05] text-center">
                            <p className="text-[7px] font-black text-zinc-500 uppercase mb-0.5">{label}</p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {Object.entries(pv.volume).slice(0, 3).map(([period, v]: [string, any]) => {
                      ...
                      return (
                        <div key={period} className="p-3 bg-[#0c0c0e] rounded-xl border border-white/[0.05] group">
                          <div className="flex justify-between items-center mb-1.5">
                            <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">{period}</span>
                            <div className="text-right">
                              <span className="text-[10px] font-black text-white italic">{v.cvol_display_text}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-1 bg-[#141416] rounded-full overflow-hidden flex">
                            ...
                            <span className={cn("text-[9px] font-black shrink-0 tabular-nums",
                              deliveryPct >= 50 ? "text-emerald-400" : deliveryPct >= 30 ? "text-violet-400" : "text-amber-400"
                            )}>{deliveryPct}% <span className="text-zinc-600 text-[7px] font-bold uppercase">Delv</span></span>
                          </div>
                        </div>
                      );
                    })}
            ...
            <Card title="Forward Estimates & Forecasts" icon={Activity}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
                  ...
                  <div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-[10px]">
                        <thead>
                          <tr className="text-zinc-500 font-black uppercase tracking-tighter border-b border-white/[0.07]">
                        ...
                        <tbody className="divide-y divide-slate-800/30">
                          {ef.eps.slice(0, 4).map((row: any, i: number) => (
                            <tr key={i}>
                              <td className="py-2 text-zinc-400 font-bold">{row.date}</td>
                              <td className="py-2 text-right text-white font-black">{row.avg || '—'}</td>
                              <td className={cn("py-2 text-right font-black", row.actual ? "text-emerald-400" : "text-zinc-600")}>{row.actual || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {ef.netProfit && ef.netProfit.length > 0 && (
                  <div>
                    <p className="text-[9px] font-black text-violet-500 uppercase tracking-widest border-l-2 border-violet-500 pl-2 mb-3">Net Profit (Cr) Est.</p>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-[10px]">
                        <thead>
                          <tr className="text-zinc-500 font-black uppercase tracking-tighter border-b border-white/[0.07]">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {hm.list.map((row: any, i: number) => (
                      <tr key={i} className="text-[11px] font-bold">
                        <td className="py-2 pr-3 text-zinc-300">{row.quarter}</td>
                        <td className="py-2 pr-3 text-right text-white">{row.actual || '—'}</td>
                        <td className="py-2 pr-3 text-right text-zinc-300">{row.estimates || '—'}</td>
                        <td className="py-2 text-right">
                          <span className={cn("text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-tighter",
                            ...
                            row.type === 'negative' ? "bg-rose-500/10 text-rose-400" : "bg-white/[0.08] text-zinc-400"
                          )}>{row.type}</span>
                        </td>
                      </tr>
                    ))}
          ...
          {detailedInsights?.financials?.piotroskiData && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-[#0c0c0e] border border-white/[0.07] rounded-2xl text-center">
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1">Piotroski Score</p>
                ...
                <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest mt-1">{detailedInsights.financials.piotroskiData.shortDesc}</p>
              </div>
              <div className="md:col-span-2 p-4 bg-[#0c0c0e] border border-white/[0.07] rounded-2xl flex items-center">
                <p className="text-[10px] text-zinc-400 italic font-medium">{detailedInsights.financials.piotroskiData.tooltip}</p>
              </div>
            </div>
          )}
            ...
            <Card title="Credit Ratings" icon={Filter}>
              <div className="space-y-2 pt-2">
                {tb.creditRating.slice(0, 6).map((cr: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between p-3 bg-[#0c0c0e] rounded-xl border border-white/[0.05]">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          {agency && <p className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">{agency}</p>}
                          <p className="text-[9px] text-zinc-500 font-bold">{[sectype, status].filter(Boolean).join(' · ')}</p>
                        </div>
                      </div>
                      {date && <p className="text-[9px] text-zinc-600 font-bold shrink-0">{String(date).slice(0, 10)}</p>}
                    </div>
                  );
                })}
              </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card 
                ...
                action={
                  <button
                    ...
                    className="px-3 py-1 bg-blue-600/20 hover:bg-blue-600/30 text-violet-400 text-[9px] font-black uppercase tracking-widest rounded-lg border border-violet-500/30 transition-all"
                  >
                ...
                <div className="space-y-4">
                  {!useTVChart && (
                    <div className="flex flex-wrap justify-between items-center gap-4">
                      <div className="flex flex-wrap gap-1 bg-[#0c0c0e] p-1 rounded-xl border border-white/[0.07]">
                        {['1d', '5d', '1m', '3m', '6m', '1y', '5y', 'max'].map((d) => (
                          <button
                            ...
                            className={cn(
                              ...
                              chartDuration === d ? "bg-blue-600 text-white shadow-lg" : "text-zinc-500 hover:text-zinc-300"
                            )}
                          >
                            {d}
                          </button>
                      ...
                      <div className="flex gap-1 bg-[#0c0c0e] p-1 rounded-xl border border-white/[0.07]">
                        {['line', 'ohlc'].map((t) => (
                          <button
                            ...
                            className={cn(
                              "px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all",
                              chartType === t ? "bg-blue-600 text-white shadow-lg" : "text-zinc-500 hover:text-zinc-300"
                            )}
                          >
                            {t}
                          </button>
                  ...
                  <div className={cn("w-full bg-[#0c0c0e]/50 rounded-2xl border border-white/[0.04] p-3 overflow-hidden", useTVChart ? "h-[450px]" : "h-[300px]")}>
                    {useTVChart ? (
                      ...
                      loadingOhlc ? (
                        ...
                        <div className="h-full flex items-center justify-center text-zinc-700 text-[10px] font-black uppercase tracking-widest italic">
              ...
              {tech && (
                <Card title={`Technical Signals (${timeframe === 'D' ? 'Daily' : timeframe === 'W' ? 'Weekly' : 'Monthly'})`} icon={Activity}>
                  <div className="space-y-6">
                    {tech.sentiments && (
                      <div className="p-3 bg-[#0c0c0e]/60 rounded-2xl border border-white/[0.07]">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <Activity className="w-3.5 h-3.5 text-violet-500" />
                            <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Sentiment Hub</span>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          {[
                            ...
                            { label: 'Neu', val: tech.sentiments.totalNeutral, color: 'text-zinc-400' },
                            { label: 'Bear', val: tech.sentiments.totalBearish, color: 'text-rose-400' },
                          ].map(s => (
                            <div key={s.label} className="p-1.5 bg-[#141416]/40 rounded-xl border border-white/[0.04] text-center">
                              <p className={cn("text-base font-black italic", s.color)}>{s.val}</p>
                              <p className="text-[6px] font-black uppercase tracking-widest text-zinc-600">{s.label}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1.5">
                          {tech.crossover?.map((cross: any) => (
                            <div key={cross.key} className="flex justify-between items-center p-2.5 bg-[#0c0c0e] rounded-xl border border-white/[0.05]">
                              <span className="text-[10px] font-black text-zinc-500 uppercase">{cross.period}</span>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-zinc-300 font-bold">{cross.displayValue}</span>
                                <SentimentBadge sentiment={cross.indication} />
                              </div>
                            </div>
                          ))}
                          {(!tech.crossover || tech.crossover.length === 0) && (
                            <p className="text-[9px] text-zinc-600 italic p-4 text-center border border-dashed border-white/[0.07] rounded-xl">No active crossovers</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <Card title="Moving Averages" icon={TrendingUp}>
                  <div className="pt-2 overflow-x-auto">
                    <table className="w-full text-[10px] text-left">
                      <thead>
                        <tr className="text-zinc-500 font-black uppercase tracking-widest border-b border-white/[0.07]">
                      ...
                      <tbody className="divide-y divide-slate-800/30">
                        {['5', '10', '20', '50', '100', '200'].map(period => {
                          ...
                          if (!s && !e) return null;
                          return (
                            <tr key={period} className="group hover:bg-[#141416]/40 transition-colors">
                              <td className="py-2.5 font-black text-zinc-400">MA {period}</td>
          ...
          <Card title="Pivot Alignment Dashboard" icon={Filter}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
              ...
              {tech?.pivotLevels && tech.pivotLevels.length > 0 && (
                <div className="space-y-4">
                  <p className="text-[9px] font-black text-violet-500 uppercase tracking-widest border-l-2 border-violet-500 pl-2">MC Classic Pivots</p>
                  {tech.pivotLevels.map((pg: any) => (
                    <div key={pg.key} className="p-3 bg-[#0c0c0e] rounded-xl border border-white/[0.05]">
                      <p className="text-[8px] font-black text-zinc-500 uppercase mb-2 text-center">{pg.key}</p>
                      <div className="grid grid-cols-5 gap-1 text-center">
                        <div className="p-1.5 bg-[#141416] rounded-lg border border-rose-500/10">
                          <p className="text-[7px] font-black text-rose-400">S2</p>
                          <p className="text-[10px] font-black text-white">₹{pg.pivotLevel.s2}</p>
                        </div>
                        <div className="p-1.5 bg-[#141416] rounded-lg border border-rose-500/5">
                          <p className="text-[7px] font-black text-rose-300">S1</p>
                          <p className="text-[10px] font-black text-white">₹{pg.pivotLevel.s1}</p>
                        </div>
                        <div className="p-1.5 bg-[#141416] rounded-lg border border-violet-500/20 ring-1 ring-violet-500/40/10">
                          <p className="text-[7px] font-black text-violet-400">PP</p>
                          <p className="text-[10px] font-black text-white">₹{pg.pivotLevel.pivotPoint}</p>
                        </div>
                        <div className="p-1.5 bg-[#141416] rounded-lg border border-emerald-500/5">
                          <p className="text-[7px] font-black text-emerald-300">R1</p>
                          <p className="text-[10px] font-black text-white">₹{pg.pivotLevel.r1}</p>
                        </div>
                        <div className="p-1.5 bg-[#141416] rounded-lg border border-emerald-500/10">
              ...
              {tb?.pivotData && (
                <div className="space-y-4">
                  ...
                  {[
                    ...
                    <div key={label} className="p-3 bg-[#0c0c0e] rounded-xl border border-white/[0.05]">
                      <p className="text-[8px] font-black text-zinc-500 uppercase mb-2 text-center">{label}</p>
                      <div className="grid grid-cols-7 gap-1 text-center">
                        {[
                          ...
                          <div key={k} className="p-1 bg-[#141416] rounded-md">
          ...
          {(technicalRating || technicalV2) && (() => {
            ...
            return (
              <div className={cn("p-4 rounded-2xl border flex items-center justify-between gap-4",
                ...
                isBear ? "bg-rose-500/5 border-rose-500/20" : "bg-[#0c0c0e] border-white/[0.07]"
              )}>
                <div className="flex items-center gap-3">
                  <Zap className={cn("w-5 h-5", isBull ? "text-emerald-400" : isBear ? "text-rose-400" : "text-zinc-500")} />
                  <div>
                    <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                      MC Technical Rating ({timeframe === 'D' ? 'Daily' : timeframe === 'W' ? 'Weekly' : 'Monthly'})
                    </p>
                    <p className={cn("text-sm font-black uppercase italic", isBull ? "text-emerald-400" : isBear ? "text-rose-400" : "text-zinc-300")}>
                      {ratingText || 'Neutral'}
                    </p>
                  </div>
                </div>
        ...
        <div className="space-y-6">
                  ...
                  <div className="space-y-1.5">
                    {(() => {
                      ...
                      return pros.slice(0, 3).map((p: string, i: number) => (
                        <div key={i} className="flex items-start gap-2 text-[10px] text-zinc-300 font-medium leading-relaxed bg-emerald-500/5 p-1.5 rounded-lg">
                          <span className="text-emerald-400 font-black">✓</span> {p}
                        </div>
                      ));
                    })()}
                    {swot?.strengths?.slice(0, 3).map((s: string, i: number) => (
                      <div key={`s-${i}`} className="flex items-start gap-2 text-[10px] text-zinc-300 font-medium leading-relaxed bg-blue-500/5 p-1.5 rounded-lg">
                        <Zap className="w-3 h-3 text-violet-400 shrink-0" /> {s}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-1.5">
                    {(() => {
                      ...
                      return cons.slice(0, 3).map((c: string, i: number) => (
                        <div key={i} className="flex items-start gap-2 text-[10px] text-zinc-300 font-medium leading-relaxed bg-rose-500/5 p-1.5 rounded-lg">
                          <AlertCircle className="w-3 h-3 text-rose-500 shrink-0" /> {c}
                        </div>
                      ));
                    })()}
                    {swot?.weaknesses?.slice(0, 3).map((w: string, i: number) => (
                      <div key={`w-${i}`} className="flex items-start gap-2 text-[10px] text-zinc-300 font-medium leading-relaxed bg-amber-500/5 p-1.5 rounded-lg">
          ...
          {(detailedInsights?.shareholding || tb?.shareHoldingGraph) && (
            <Card title="Shareholding Intelligence" icon={PieChart}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-2">
                <div className="bg-[#0c0c0e]/60 border border-white/[0.07] rounded-2xl p-3">
                  <p className="text-[9px] font-black text-violet-500 uppercase tracking-widest mb-3 border-l-2 border-violet-500 pl-2">Ownership Dynamics</p>
                  <div className="space-y-1.5">
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="grid grid-cols-5 gap-1.5">
                      {ar.ratings?.map((r: any, i: number) => {
                        ...
                        return (
                          <div key={i} className="text-center p-1.5 bg-[#141416]/40 rounded-lg border border-white/[0.04]">
                    ...
                    {ar.analystCount && (
                      <p className="text-[8px] text-zinc-600 italic text-center font-bold uppercase tracking-widest mt-2">
                ...
                <Card title="Forward Price Targets" icon={TrendingUp}>
                  <div className="grid grid-cols-3 gap-2 pt-1">
                    {[
                      ...
                      { label: 'Mean', val: pf.mean, color: 'text-violet-400',    bg: 'bg-blue-500/5'    },
                      ...
                      <div key={t.label} className={cn("p-2 rounded-xl border border-white/5 text-center", t.bg)}>
                        <p className="text-[7px] font-black text-zinc-500 uppercase tracking-widest mb-1">{t.label}</p>
                        <p className={cn("text-sm font-black italic", t.color)}>₹{t.val}</p>
                      </div>
                    ))}
                  </div>
                  <div className="mt-3 h-1 w-full bg-[#141416] rounded-full overflow-hidden flex">
          ...
          {consensus && (
            <Card title="Analyst Consensus Trend" icon={Users}>
              <div className="space-y-3 pt-2">
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-2">Rating Distribution Over Time</p>
                <div className="flex flex-wrap gap-2">
                  {consensus.categories?.map((cat: any, i: number) => (
                    <span key={i} className="text-[9px] font-bold text-zinc-400 bg-[#0c0c0e] px-2 py-1 rounded border border-white/[0.07]">{cat}</span>
                  ))}
                </div>
                  ...
                  {consensus.graphData?.map((g: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-2 bg-[#0c0c0e] rounded-full overflow-hidden border border-white/[0.07]">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {valuation.list.map((row: any, i: number) => (
                      <tr key={i} className="text-[11px] font-bold hover:bg-[#141416]/50 transition-colors">
                        <td className="py-2.5 pr-3 text-violet-400 font-black uppercase tracking-widest">{row.heading}</td>
                        <td className="py-2.5 pr-3 text-right text-white tabular-nums">{row.data?.eps || '—'}</td>
                        <td className="py-2.5 pr-3 text-right text-zinc-300 tabular-nums">{row.data?.pe || '—'}</td>
                        <td className="py-2.5 pr-3 text-right text-zinc-300 tabular-nums">{row.data?.bvps || '—'}</td>
                        <td className="py-2.5 pr-3 text-right text-zinc-300 tabular-nums">{row.data?.pb || '—'}</td>
                        <td className="py-2.5 text-right">
      ...
      {activeTab === 'trendlyne' && (
        <div className="space-y-6">
          {loadingTlMetrics || loadingTlTa ? (
            <div className="flex items-center justify-center p-8 bg-[#141416]/10 border border-white/[0.07] border-dashed rounded-2xl">
              <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 animate-pulse">Loading Trendlyne Data...</span>
            </div>
          ) : (
            <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
                    {Object.entries(trendlyneMetrics.body).filter(([key]) => key !== 'prepend_params' && key !== 'head').map(([key, metric]: [string, any]) => (
                      <div key={key} className={cn("p-3 rounded-xl border flex flex-col justify-between",
                        ...
                        <div>
                          <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1 leading-tight">{metric.title}</p>
                          <p className="text-[11px] text-zinc-300 mb-1">{metric.st1 || metric.st2 || '—'}</p>
                        </div>
                ...
                <Card title="Advanced Technical Analysis (Trendlyne)" icon={Activity}>
                  <div className="space-y-6 pt-2">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="bg-[#0c0c0e] p-3 rounded-xl border border-white/[0.07] flex flex-col items-center justify-center">
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">RSI (14)</span>
                        <span className={cn("text-xl font-black italic mt-1", trendlyneTa.body.parameters.rsi?.color === 'positive' ? 'text-emerald-400' : trendlyneTa.body.parameters.rsi?.color === 'negative' ? 'text-rose-400' : 'text-zinc-300')}>{trendlyneTa.body.parameters.rsi?.value}</span>
                        <span className="text-[8px] text-zinc-400 text-center mt-1">{trendlyneTa.body.parameters.rsi?.insight?.shorttext}</span>
                      </div>
                      <div className="bg-[#0c0c0e] p-3 rounded-xl border border-white/[0.07] flex flex-col items-center justify-center">
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">MACD</span>
                        <span className={cn("text-xl font-black italic mt-1", trendlyneTa.body.parameters.macd?.color === 'positive' ? 'text-emerald-400' : trendlyneTa.body.parameters.macd?.color === 'negative' ? 'text-rose-400' : 'text-zinc-300')}>{trendlyneTa.body.parameters.macd?.value}</span>
                        <span className="text-[8px] text-zinc-400 text-center mt-1">{trendlyneTa.body.parameters.macd?.insight?.shorttext}</span>
                      </div>
                      <div className="bg-[#0c0c0e] p-3 rounded-xl border border-white/[0.07] flex flex-col items-center justify-center">
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Momentum</span>
                        <span className={cn("text-xl font-black italic mt-1", trendlyneTa.body.parameters.momentum?.color === 'positive' ? 'text-emerald-400' : trendlyneTa.body.parameters.momentum?.color === 'negative' ? 'text-rose-400' : 'text-zinc-300')}>{trendlyneTa.body.parameters.momentum?.value}</span>
                        <span className="text-[8px] text-zinc-400 text-center mt-1">{trendlyneTa.body.parameters.momentum?.insight?.shorttext}</span>
                      </div>
                      <div className="bg-[#0c0c0e] p-3 rounded-xl border border-white/[0.07] flex flex-col items-center justify-center">
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">MFI</span>
                        <span className={cn("text-xl font-black italic mt-1", trendlyneTa.body.parameters.mfi?.color === 'positive' ? 'text-emerald-400' : trendlyneTa.body.parameters.mfi?.color === 'negative' ? 'text-rose-400' : 'text-zinc-300')}>{trendlyneTa.body.parameters.mfi?.value}</span>
                        <span className="text-[8px] text-zinc-400 text-center mt-1">{trendlyneTa.body.parameters.mfi?.insight?.shorttext}</span>
                      </div>
                    </div>
                    {trendlyneTa.body.parameters.ma_signal && (
                      <div className="bg-[#0c0c0e] p-4 rounded-xl border border-white/[0.07]">
                        <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Moving Averages Signal</p>
                        <div className="flex gap-4 items-center">
                          <div className="flex-1 text-center">
                            <span className="block text-xl font-black text-emerald-400">{trendlyneTa.body.parameters.ma_signal.bullish}</span>
                            <span className="text-[9px] text-zinc-500 uppercase">Bullish</span>
                          </div>
                          <div className="flex-1 text-center">
                            <span className="block text-xl font-black text-rose-400">{trendlyneTa.body.parameters.ma_signal.bearish}</span>
                            <span className="text-[9px] text-zinc-500 uppercase">Bearish</span>
                          </div>
                        </div>
                        <p className="text-[9px] text-center mt-3 text-zinc-400">{trendlyneTa.body.parameters.ma_signal.sma_insight}</p>
                        <p className="text-[9px] text-center text-zinc-400">{trendlyneTa.body.parameters.ma_signal.ema_insight}</p>
                      </div>
                    )}
                  </div>
                </Card>
              ...
              {!(trendlyneMetrics as any)?.body && !(trendlyneTa as any)?.body?.parameters && (
                <div className="text-center p-8 bg-[#0c0c0e] border border-white/[0.07] rounded-2xl">
                  <p className="text-zinc-500 text-sm font-bold">No Trendlyne data available for this stock.</p>
                </div>
              )}
            </>
          )}
      ...
      {/* ── Footer ── */}
      <div className="text-center pt-4 border-t border-white/[0.07]">
