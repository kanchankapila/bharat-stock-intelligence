...
export const TradeDecisionCockpit: React.FC<{ onSelectStock: (symbol: string) => void }> = ({ onSelectStock }) => {
  ...
  const overview = cockpitData?.marketOverview || { verdict: 'NO TRADE', verdictReason: 'Loading...', advDecRatio: 1.0, avgWinProbability: 50.0, activeSignalsCount: 0 };
  const candidates = cockpitData?.candidates || [];

  // Automatically select the top candidate initially
