  ...
  <div className={cn("bg-[#141416]/80 border border-white/[0.07] rounded-2xl overflow-hidden", className)}>
    {title && (
      <div className="px-4 py-3 border-b border-white/[0.07] flex items-center justify-between">
        <h3 className="text-[11px] font-black text-zinc-300 flex items-center gap-2 italic uppercase tracking-widest">
          {Icon && <Icon className="w-3.5 h-3.5 text-violet-500" />}
          {title}
        </h3>
        {action ? action : <Info className="w-3.5 h-3.5 text-zinc-600 cursor-help" />}
      </div>
    )}
...
export const ValueDisplay: React.FC<{ label: string; value: string | number | undefined; sub?: string; color?: string }> = ({ label, value, sub, color }) => (
  <div className="p-3 bg-[#0c0c0e] rounded-xl border border-white/[0.05] text-center">
    ...
    {sub && <p className="text-[8px] text-zinc-600 font-bold uppercase mt-1">{sub}</p>}
  </div>
);

export const IndicatorRow: React.FC<{ name: string; value: string | number | any[] | undefined; sentiment: string }> = ({ name, value, sentiment }) => (
  <div className="flex items-center justify-between p-2 bg-[#0c0c0e] rounded-lg border border-white/[0.05] hover:border-white/[0.1] transition-all">
    <div className="flex-1 min-w-0">
      <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest truncate">{name}</p>
...
export const CompactMetricCard: React.FC<{ label: string; value: string | number | undefined; sub?: string; color?: string; icon?: any }> = ({ label, value, sub, color, icon: Icon }) => (
  <div className="p-2.5 bg-[#0c0c0e]/40 rounded-xl border border-white/[0.05] flex flex-col justify-between hover:bg-[#141416]/60 transition-colors group">
    <div className="flex items-center justify-between mb-1">
      <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest truncate max-w-[80%]">{label}</span>
      {Icon && <Icon className="w-2.5 h-2.5 text-zinc-700 group-hover:text-zinc-500 transition-colors" />}
    </div>
    <div className="flex items-baseline gap-1.5 flex-wrap">
      <span className={cn("text-[11px] font-black italic", color || "text-zinc-200")}>{value ?? '—'}</span>
      {sub && <span className="text-[7px] text-zinc-600 font-bold uppercase tracking-tighter truncate">{sub}</span>}
    </div>
  </div>
);
