    ...
    <section className="v5-grid">
      ...
      <div className="v5-card col-span-12 p-4 lg:col-span-4">
        ...
        <div className="v5-compact-scroll mt-3 space-y-2 pr-1">
          {searchRows.slice(0, 30).map((row: any) => {
            ...
            return (
              <button
                ...
                className={`v5-table-row-intel w-full rounded-xl border px-3 py-2 text-left transition ${selectedSymbol === sym ? 'border-[var(--v5-accent)] bg-[var(--v5-accent-soft)]' : 'border-[var(--v5-border)] bg-[var(--v5-surface)] hover:bg-[var(--v5-surface-2)]'}`}
              >
          ...
          {!searchRows.length && <p className="text-sm text-[var(--v5-muted)]">Start typing to search symbols.</p>}
        </div>
      </div>
      ...
      <div className="v5-card col-span-12 p-4 lg:col-span-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[var(--v5-muted)]">Execution-grade snapshot for intraday + swing context</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-[var(--v5-surface-2)] px-2 py-1 text-xs font-semibold text-[var(--v5-ink-soft)]">Data-linked</span>
            <span className="rounded-full bg-[var(--v5-accent-soft)] px-2 py-1 text-xs font-semibold text-[var(--v5-accent-deep)]">Live DB</span>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-[var(--v5-border)] bg-[var(--v5-surface-2)] p-3">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--v5-muted)]">LTP</div>
            <div className="mt-1 text-xl font-bold">{quotePrice == null ? '—' : `₹${fmtINR(quotePrice)}`}</div>
          </div>
          <div className="rounded-2xl border border-[var(--v5-border)] bg-[var(--v5-surface-2)] p-3">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--v5-muted)]">Day Change</div>
            <div className={`mt-1 text-xl font-bold ${pctClass(dayChangePct ?? 0)}`}>
              {dayChangePct == null ? '—' : `${dayChangePct >= 0 ? '+' : ''}${fmtFixed(dayChangePct, 2)}%`}
            </div>
          </div>
          <div className="rounded-2xl border border-[var(--v5-border)] bg-[var(--v5-surface-2)] p-3">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--v5-muted)]">Unified Score</div>
            <div className="mt-1 text-xl font-bold">{unifiedScore == null ? '—' : fmtFixed(unifiedScore, 1)}</div>
          </div>
          <div className="rounded-2xl border border-[var(--v5-border)] bg-[var(--v5-surface-2)] p-3">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--v5-muted)]">Classification</div>
            <div className="mt-1 text-lg font-bold text-[var(--v5-ink)]">{s((detailQ.data as any)?.classification, '—')}</div>
          </div>
        </div>

        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-[var(--v5-border)] p-3">
            <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-[var(--v5-ink-soft)]"><ShieldCheck className="h-4 w-4 text-[var(--v5-positive)]" /> Why this setup</div>
            <p className="text-sm leading-relaxed text-[var(--v5-ink-muted)]">{s((detailQ.data as any)?.whyThisPick, 'Reasoning not available for this symbol/timeframe yet.')}</p>
          </div>
          <div className="rounded-2xl border border-[var(--v5-border)] p-3">
            <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-[var(--v5-ink-soft)]"><Globe2 className="h-4 w-4 text-sky-700" /> Market Context</div>
            <ul className="space-y-1 text-sm text-[var(--v5-ink-muted)]">
