...
| AF-20260817-24 | 2026-08-17 | monitoring | **Two undocumented threshold constants gate live demotions in the canonical ranker with no measured-null derivation.** `unified_ranker.py:502` `CORRELATION_CLUSTER_THRESHOLD = 0.65` (pairwise-correlation cluster cut, caps position-sizing exposure) and `unified_ranker.py:981` `FACTOR_CROWDING_THRESHOLD = 0.70` (gates a live 0.90× demotion on `unified_recommendations`) both have comments explaining the *mechanism* but not why that specific cut point was chosen — same shape `drift_detector.py`'s `PSI_CRIT` was in before it was recalibrated against a measured null distribution (now documented in its own comment). Separately, `backtest_optimizer.py`'s promotion gate (`UPDATE_THRESHOLD = 1.05` — "new Sharpe must be ≥ current × 1.05") has no run-to-run noise check, the same class `recurring-bugs.md`'s HMM-regime-retrain entry already burned once ("a champion/challenger gate is meaningless if seed noise exceeds the gap"). Found by `/threshold-calibration-audit` (Lane 6, rotation group 4). | EVIDENCE | **deliberately NOT fixed** — picking a new number for a threshold that gates live scoring/promotion without a real `factor_backtest.py`-style measurement is exactly `recurring-bugs.md`'s "fabricated backtest" / unmeasured-scoring-change failure mode, the single most repeated failure in this repo's history, and `verify-gate.mjs` blocks completion on it for good reason. Documenting the gap (this row) is the honest stopping point for a mechanical audit pass; deriving a real threshold is its own measurement session | — | — |
