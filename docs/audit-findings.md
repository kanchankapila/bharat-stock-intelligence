...
| AF-20260817-23 | 2026-08-17 | monitoring | **`dq-uninformative-checks` only watches for stuck-BAD verdicts, never stuck-GOOD.** Its SQL filters `distinct_status = 1 AND only_status <> 'pass'` — a check that is structurally incapable of ever firing non-pass (too loose a threshold, an unreachable condition) is invisible to this meta-check no matter how many runs accumulate, same blind-spot shape as the "monitor that fires 16/16" class in `recurring-bugs.md`, inverted. Confirmed live: of ~152 checks over a 109-run/~2-day window, 147 were `pass`-only — expected for genuinely healthy fetchers over 2 days, but the meta-check has no mechanism to ever distinguish "healthy" from "structurally can't fail" even given 30+ days of history. Found by `/threshold-calibration-audit` (Lane 6, rotation group 4). | EVIDENCE | **open** — needs a longer observation window (30+ days, matching the meta-check's own lookback) before any specific one of the 147 could be named as structurally-broken rather than currently-healthy; the architectural gap itself (stuck-good is unwatched) is real regardless | — | — |
| AF-20260817-24 | 2026-08-17 | monitoring | **Two undocumented threshold constants gate live demotions in the canonical ranker with no measured-null derivation.** `unified_ranker.py:502` `CORRELATION_CLUSTER_THRESHOLD = 0.65` (pairwise-correlation cluster cut, caps position-sizing exposure) and `unified_ranker.py:981` `FACTOR_CROWDING_THRESHOLD = 0.70` (gates a live 0.90× demotion on `unified_recommendations`) both have comments explaining the *mechanism* but not why that specific cut point was chosen — same shape `drift_detector.py`'s `PSI_CRIT` was in before it was recalibrated against a measured null distribution (now documented in its own comment). Separately, `backtest_optimizer.py`'s promotion gate (`UPDATE_THRESHOLD = 1.05` — "new Sharpe must be ≥ current × 1.05") has no run-to-run noise check, the same class `recurring-bugs.md`'s HMM-regime-retrain entry already burned once ("a champion/challenger gate is meaningless if seed noise exceeds the gap"). Found by `/threshold-calibration-audit` (Lane 6, rotation group 4). | EVIDENCE | **open** — changing a threshold that gates live scoring/promotion needs a `factor_backtest.py`-style measurement first, per this repo's own `verify-gate.mjs` mandate; not something to fix mechanically | — | — |
| AF-20260817-25 | 2026-08-17 | monitoring | `regime_edge_status` (backs `regime-edge-trust-floor`, one of the 3 known stuck-warn checks) is a **6-row overwrite-in-place snapshot last recomputed 2026-08-14** — `count(*)=6, count(DISTINCT computed_at)=1` across the full 109-run query window. The warn verdict itself is arithmetically correct and not a monitoring bug (`HIGH_VOL` AUC 0.4776 < 0.55 floor, consistent with `measurement.md`'s already-documented significantly-negative `high_vol` factor, t=−6.09) — but the snapshot hasn't been recomputed in 3+ days, and nothing in `TABLE_FRESHNESS_CHECKS` watches it (it's hand-rolled "ML state," explicitly out of the freshness mandate's scope per `data-sources.md`). If its refresh job silently stopped, staleness itself would become the real defect underneath a verdict that reads as correct today. Found by `/threshold-calibration-audit` (Lane 6, rotation group 4). | INVESTIGATE | **open** — needs tracing which job/script recomputes `regime_edge_status` and confirming it's still scheduled; not established this pass | — | — |
