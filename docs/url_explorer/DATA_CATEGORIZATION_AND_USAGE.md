...
| `api.moneycontrol.com/swiftapi/v1/stockvitals/historical` (`metric=altman/ohlson/graham/dupont`) | **New**, not in registry | `data.barData.{year,value}` — 10-year bar-chart series per model | MC's own **historical trend** of 4 distinct bankruptcy/quality models. This codebase already computes Altman Z/Piotroski independently (`ml_ensemble.py`) — same caution as `investsights_score`'s forensic_scores: cross-validate before adding a second copy, but the Ohlson/Dupont pair (not independently computed anywhere in this codebase per my check) are new signal. Frontend: a "Financial Health Trend" mini-chart on the Fundamentals tab, showing how these 4 scores evolved over 10 years. |
...
| `api.moneycontrol.com/mcapi/v1/{cat}/scanner-detail?catId&scanId` (**220 URLs → 1 template**) | **New** — not wired anywhere found | `data.list.scannerDetails.columns.{name,value}`, `scannerName`, `scannerDescription` | This is MoneyControl's own screener/scanner catalog (220 distinct `scanId`s captured) — structurally identical in purpose to what `trendlyne_screener_discovery.py` does for Trendlyne and `etMarketstats.ts` does for ET, but for MC's scanner surface, which this codebase does **not** currently ingest as a screener source. Given `screener_master`/`confluenceEngine.ts` already generalize across 4 screener providers (Trendlyne/MC-basic/ETnow/ET-Marketstats), this is a natural 5th-ish addition — but per this codebase's own 2026-07-30/31 audit findings (screener weight already dominant and *wired backwards* on momentum), **do not add a 5th screener source without first re-running the `factor_edge.py`-style forward-return check** the audit demands before trusting any new screener's signal. |
