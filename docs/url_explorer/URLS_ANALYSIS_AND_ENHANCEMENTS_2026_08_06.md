...
**Update (same day, follow-up pass): item 1 below turned out to already be done — by a different, concurrent session, via a better mechanism than the one this doc originally proposed.** Before wiring `sector_correlation_pairs` into `unified_ranker.py`, a check of the live source found `cluster_by_correlation()`/`apply_correlation_cap()` already committed (`fcd8557`, "portfolio correlation cap") and already active in `run()`'s position-sizing path — an *empirical, stock-level* pairwise return-correlation clustering (union-find over `stock_ohlcv`-derived returns, 60-day lookback, date-keyed alignment so mismatched trading-day gaps can't corrupt the estimate), which is more rigorous than the sector-level, third-party-sourced matrix this doc's own recommendation would have used: it catches cross-sector correlation (e.g. two same-factor names with different GICS labels) that a sector-tag-based cap structurally cannot. **It shipped with zero test coverage** — closed with 14 new regression tests in `test_unified_ranker_portfolio_construction.py`, negative-controlled (breaking the union-find operation makes the transitive-grouping tests fail with the exact expected symptom). No changes were made to the live sizing algorithm itself — it was already correct; this pass only added the missing test coverage.

`sector_correlation_pairs`/`sector_rrg_history` remain genuinely useful for what they were actually built for — the frontend RRG chart and sector-level correlation view (still not superseded by the stock-level cap, which produces opaque numeric cluster ids, not a human-readable sector narrative) — but are not, and should not be, wired into position sizing a second way alongside the mechanism that already exists there.

**Item 5 (`www.ndtvprofit.com`) also resolved this pass**: re-tested properly across 6 real symbols (RELIANCE, TCS, HDFCBANK, INFY, SBIN, DRCSYSTEMS) and 3 endpoint families (`stocks/dashboard`, `stock-summary` futures-basis, `stocks/announcements`) using the correct `/api/v2/` path (the earlier anecdotal test's path was under-specified). **Confirmed reliably alive**, 18/18 real requests returned genuine, symbol-scoped, current-looking data (DRCSYSTEMS' stock-summary correctly returned all-zero/null fields — that stock has no active F&O contract, not a fetch failure). The 2026-07-30 "confirmed dead" finding is superseded — `curl_cffi` Chrome-TLS impersonation is required, a plain `requests` call is not enough for this host. **Not yet built into a fetcher** — that's a new, separate scope decision (its own table, scheduling, tests, `dataQualityChecks.ts` entry) that should be confirmed with the user before starting, not assumed.
...
| 5 | Build a real fetcher on `www.ndtvprofit.com` (futures basis/roll-spread cross-check + per-stock corporate announcements) | **Confirmed alive this pass** (see above) — no longer a verification question, now a scope question: needs its own table, scheduling cadence, `dataQualityChecks.ts` entry, and a decision on whether the futures-basis data is worth cross-validating against `fno_rollover_fetcher.py`'s existing NSE-bhavcopy-derived basis before trusting it as a second source. Confirm with the user before starting. |
